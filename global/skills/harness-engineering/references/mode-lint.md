...
| **Trigger alias** | Does description include `Trigger:` aliases where model invocation is enabled? | Add the actual slash commands; `bin/check` enforces frontmatter shape. |
...
rg -n 'source.*\$REPO_ROOT|source.*/scripts/lib/' global/skills/*/scripts/

# Scripts that walk up past global/skills/<name>/ via $SCRIPT_DIR/../..
rg -n 'SCRIPT_DIR/\.\./\.\.' global/skills/*/scripts/
```
...
Run on all skills: `for s in global/skills/*/SKILL.md; do /harness-engineering lint "$s"; done`
