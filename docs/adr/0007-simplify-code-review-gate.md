...
3. **Rigid 3-pass requirement:** `review_receipt.py` requires exactly three fixed reviewers (`autoreview`, `thermo-nuclear-review`, `thermo-nuclear-code-quality-review`). This leaves the `ponytail` critic (mandated by `global/AGENTS.md:33`) un-gated and forces prose changes to run code-heavy reviewers.
...
- Enforces `ponytail` complexity audit on all substantive code changes per `global/AGENTS.md:33`.
