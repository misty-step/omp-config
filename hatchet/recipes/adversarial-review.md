...
7. After the final tool result, you MUST send one final ordinary assistant text message and must not end on a tool call. That message must be only a compact JSON object with this exact shape: `{"version":1,"outcome":"accepted|blocked","headSha":"<full SHA>","artifactRefs":["<evidence>"]}`. Do not call the `yield` tool. Do not wrap the object in Markdown. Use outcome `accepted` only when no blocker remains; otherwise use `blocked`. Include each concrete finding or exact passing command and `worktree:unchanged` in `artifactRefs`.
