...
`global/skills/design/references/external/` and loadable by alias; run them, do not
paraphrase them.
...
| `zeke-swiss-design` (installed) | Swiss International Style as a blind bench lane: 12-col grid, IBM Plex Sans, stone palette with opacity-only hierarchy, single accent color, rectilinear geometry | MIT. Tailwind-flavored token names; port the ratios (60ch measure, py-16 minimum, one accent at ≤15% coverage) to the target stack rather than requiring Tailwind. |
| `dammyjay-interface-design` (installed) | Dense operational UI as a blind bench lane: domain-exploration brief (human/task/feel), focal-point hierarchy, subtle-layering craft (surface elevation, border opacity), token architecture | MIT. Explicitly out of scope for marketing/landing pages — route those to hallmark, soft, brutalist, swiss-design, or compact-landing instead. |
| `danilaa-compact-landing` (installed) | Small dev-tool/product landing pages as a blind bench lane: an explicit "design fingerprint" (composition, measure, palette, type, radius, surface grammar, motif, motion) mutated at least four axes per run to avoid one frozen template | MIT. Ships an optional `scripts/pick-direction.mjs` randomizer — treat its output as a creative constraint, not a command; still hand-adapt to the fence's actual content. |
