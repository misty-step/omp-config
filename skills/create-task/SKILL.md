...
> Vendored from Harbor Framework's official [`create-task` skill](https://github.com/harbor-framework/harbor/blob/ac398bbda7c4c1073461797d3b95c2455cc671b5/skills/create-task/SKILL.md) under Apache-2.0.
> OMP contributors added this notice, updated the stale task-init command and its flags, fixed an invalid shell continuation, trimmed upstream trailing whitespace, and replaced repository-relative or unavailable cross-skill references with absolute official links.
...
judge, or would benefit from composable reusable checks. See the [Reward Kit documentation](https://www.harborframework.com/docs/rewardkit/motivation).
...
Then add `tests/checks.py` and/or `tests/judge.toml`. Use the [Reward Kit documentation](https://www.harborframework.com/docs/rewardkit/motivation) to design the criteria.
...
harbor run -p "<task-path>" -a oracle \
  ...
  --allow-agent-host files.pythonhosted.org
```

On a `public` baseline, run-time host flags emit a warning and are ignored.

See the [network-policy example](https://github.com/harbor-framework/harbor/tree/ac398bbda7c4c1073461797d3b95c2455cc671b5/examples/tasks/network-policy-matrix) and the official [Network policy reference](https://www.harborframework.com/docs/tasks#network-policy).
...
`harbor init --task` leaves `README.md` as a stub. Before wrapping up, populate it so
...
- [Multi-step task documentation](https://www.harborframework.com/docs/tasks/multi-step)
- [Advanced multi-step example](https://github.com/harbor-framework/harbor/tree/ac398bbda7c4c1073461797d3b95c2455cc671b5/examples/tasks/hello-multi-step-advanced)
