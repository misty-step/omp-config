...
The Hatchet worker runs a bounded GC every five minutes for `~/.omp/wt` (overridable with `OMP_WORKTREE_ROOT`). The collector refuses to run unless the resolved root ends in `.omp/wt` or is listed in `OMP_WORKTREE_ROOT_ALLOW` (colon-separated, realpath-resolved), so a misconfigured or attacker-set root can never turn the periodic GC into a recursive-delete primitive over an arbitrary subtree. A worktree is a removal candidate only when it is not in use (`lsof`) and `git status` proves it clean; a tree whose git status is unreadable or times out (e.g. a slow `git status` on a very large clone, or a dangling gitdir pointer) is treated as protected (fail-closed) — the collector never deletes a tree it cannot prove clean, only one it can prove clean. Candidates older than 15 minutes (measured by the most recent file mtime inside the tree, not the top-level directory mtime, so file edits keep a tree fresh) are removed; the collector also enforces an eight-worktree or 20 GiB logical-size budget and reports any budget it cannot satisfy because a tree is dirty, unreadable, or in use. Liveness and cleanliness are re-verified immediately before each removal, and each removed tree's path is realpath-checked against the root and refused if it is a symlink or non-directory. Removal is followed by a best-effort `git worktree prune` in the parent repo, with the parent gitdir resolved relative to the worktree (not the GC process's cwd). `OMP_WORKTREE_MAX_COUNT`, `OMP_WORKTREE_MAX_BYTES`, and `OMP_WORKTREE_GC_STALE_AFTER_MS` tune the limits; GC runs are guarded against overlapping a still-running pass.
