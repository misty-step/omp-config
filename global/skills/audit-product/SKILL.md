...
Dispatch through `skill://dispatch`. Name the native agent, task-specific skill, scope, oracle, evidence, and non-goals.
...
3. **Audit.** Run both [`references/qa-runbook.md`](references/qa-runbook.md) tracks:
   - Deterministic: a fresh, non-mutating `verifier` with `verify-live` runs each scenario live and returns `PASS`, `WARN`, `FAIL`, or `SKIP` with named evidence.
...
5. **Remediate.** Route independent gaps to `builder`; send a ranked packet to one `builder` with a two-round cap. Never mutate in a user session. Apply `fix-and-pr` authorization in `skill://qa-users`; respect the work-ledger rule. Require landed change or blocker evidence.
6. **Verify.** A fresh, non-mutating `verifier` with `verify-live` reruns each failing scenario with the same steps, oracle, and live entrypoint; close on live `PASS`. Redispatch personas for changed paths. A destroyed strength is a regression, not closure.
