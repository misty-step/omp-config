...
- Audited surfaces: `global/skills/{design,improve-ui,baseline-ui,fixing-accessibility,fixing-motion-performance,image-gen,qa-users,dispatch}`
...
| 2 | source | whole stack | No interaction-sound doctrine; only rule was "avoid autoplaying media with sound" | grep sound/audio/autoplay/mute across the 8 stack skills → fixing-accessibility §8 only | Contract: Sound rules | Add `## Sound` section to baseline-ui per Cuelume doctrine | accepted | verified |
...
Preserved strengths (no finding): design's Not-list already rejects purple-gradient SaaS and generic defaults; baseline-ui already bans purple and multicolor gradients; fixing-accessibility already bans autoplaying media with sound; image-gen already blocks raster mockups from shipping as UI.
