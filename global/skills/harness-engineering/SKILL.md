...
Discover agents in this order: project `.omp/agents`, user
`~/.omp/agent/agents`, enabled Claude-plugin `agents/`, then bundled `task`,
`scout`, `librarian`, `designer`, and `sonic`. Discover skills through analogous
native-provider precedence: project and user `.omp` or `.agents` first. Auto-learn
skills defer to same-named authored skills. No resolved bundle or catalog
projection exists beyond lookup. Repo-local vendoring is exceptional.
