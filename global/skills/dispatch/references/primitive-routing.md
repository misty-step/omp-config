...
| accepted implementation | `hephaestus` | `deliver`, `ci`, `diagnose`; add domain skills named below | `scully`; add `argus` for high-risk changes |
...
| code-review program | `argus` | `code-review`, `peer-harnesses`, `ci`, `diagnose` | cross-family reviewers plus `scully` for live claims |
...
| live verification | `scully` | `ci`, `diagnose`; add surface-specific skills | a different model family from the author |
| production incident or digital forensics | `cassandra` | `diagnose`, `factory-apps`, `estate-infrastructure`, `mint` | replay the original failure and watch the live signal |
...
| investigate a reproducible code failure | `diagnose` | `hephaestus` for local fixes; `cassandra` for live incidents |
...
| produce a rendered report | `artifact` | ad-hoc authoring lane; verify in a browser |
| create public proof and launch material | `showcase`, `artifact`, `human-writing` | `hephaestus` plus `designer` and `scully` |
| create or repair documentation | `document`, `human-writing` | `magellan` sources; `hephaestus` writes; `cerberus` checks claims |
