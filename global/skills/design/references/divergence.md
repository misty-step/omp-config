...
| **UX flow** (not UI) | the user's job-to-be-done, the required steps/states, entry & exit points | flow structure (wizard vs single-screen vs progressive disclosure vs inline), step count, where branches/decisions sit, optimistic vs confirmed, recovery/error paths | clickable step sequences or flow diagrams (mermaid/boxes) per variant — the artifact shows the *path*, not one pretty screen. Route to `curator` for `/groom` when product direction itself is unsettled |
