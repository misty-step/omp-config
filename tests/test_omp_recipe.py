...
class OmpRecipeTests(unittest.TestCase):
    ...
    def test_skill_path_roots_are_documented(self) -> None:
        ...
        self.assertIn("`taskSkills[].path` remains a raw path in `recipe.json` and resolves at run time against the target repository checkout. The compiler also validates it under the spec root but leaves it uncopied.", documentation)
