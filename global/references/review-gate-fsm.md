...
| `frozen` | `submit` clean pass for required reviewer | `passes_partial` or `passes_complete` |
...
- `verify` or ship claim from `idle` / `frozen` / `passes_partial`
- `record` before all required clean passes
- `submit` from `idle` or before freeze
