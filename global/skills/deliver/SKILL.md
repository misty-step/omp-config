...
Read `../deliver-core/SKILL.md` before starting. That linked skill is the
canonical delivery contract; this adapter keeps OMP's composer, route, review,
gate, and Powder closeout surfaces local.
...
The OMP `/dispatch` runtime remains local; do not move its composition or
recipe behavior into the canonical core.
...
Route only when the corresponding OMP oracle is needed. For disputed proof
loops, load `global/references/verification-system-first.md`. For public API,
...
review, and exact-proof requirements. OMP then:
...
- commits by concern and pushes when the OMP workflow requires it, stopping at
  merge-ready unless the operator explicitly asks for merge or deploy.
