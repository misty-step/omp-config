...
- **Rust:** Run a real complexity analyzer (`rust-code-analysis-cli`, `lizard`) in CI, or gate Clippy's supported structural proxies (`clippy::excessive_nesting`, `clippy::too_many_lines`, `clippy::type_complexity`). Do not treat Clippy's `cognitive_complexity` restriction lint as a complexity measurement tool.
